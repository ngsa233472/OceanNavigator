...
void update_position(json& state) {
    ...
    double new_lat_rad = asin(sin(lat_rad) * cos(distance_km / EARTH_RADIUS_KM) +

                              cos(lat_rad) * sin(distance_km / EARTH_RADIUS_KM) * cos(heading_rad));



    double new_lon_rad = lon_rad + atan2(sin(heading_rad) * sin(distance_km / EARTH_RADIUS_KM) * cos(lat_rad),

                                         cos(distance_km / EARTH_RADIUS_KM) - sin(lat_rad) * sin(new_lat_rad));
